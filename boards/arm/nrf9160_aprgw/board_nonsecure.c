...
static int aprgw_magpio_configure(void)
{
#if defined(CONFIG_BSD_LIBRARY) && defined(CONFIG_NET_SOCKETS_OFFLOAD)
	int at_socket_fd;
	int buffer;
...
static int aprgw_board_init(struct device *dev)
{
	int err;

	err = aprgw_magpio_configure();
	if (err) {
		LOG_ERR("aprgw_magpio_configure failed with error: %d", err);
		return err;
	}
...
SYS_INIT(aprgw_board_init, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
