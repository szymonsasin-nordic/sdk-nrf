...
static void append_to_out_buffer(const void *append_data, int len)
{
...
/* TODO: Resolve the compiler warning "note: expected 'void *' but argument
 * is of type 'const void * const'"
 */
static void send_fragment(const void *buf, size_t size)
{
...
static void uart_cb(const struct device *x, void *p)
{
	...
	while (true) {
		...
		if (strncmp(&input_buffer.buffer[bytes_recv - 3], magic_stop, 4)
		    == 0) {
			input_buffer.offs = bytes_recv;
			bytes_recv = 0;
			printk("Magic stop received, current transmission is"
			       "complete\n");
...
int dfu_target_uart_init(size_t file_size, dfu_target_callback_t cb)
{
	...
	/* TODO: If device_get_binding() fails, an error should be returned
	...
	/* The program will wait here until it receives a message that ends
	 * with magic_stop
	 * TODO: Might be smart to implement some kind of timeout that
	 * triggers after x seconds, and then this function returns an
	 * error
	 */
	...
	/* if (input_fields.active_func == INIT) {
		...
		printk("9160: Did not receive correct function INIT, "
...
int dfu_target_uart_offset_get(size_t *out)
{
	...
	/* The program will wait here until it receives a message that ends
	 * with magic_stop
	 * TODO: Maybe you should implement some kind of timeout that
	 * triggers after e.g. 5 seconds, and then this function returns an
	 * error
	 */
...
/* TODO: Resolve the warning warning: passing argument 1 of 'send_fragment'
