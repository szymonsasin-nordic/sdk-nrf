...
static bool encode_repeated_pvt_spd(
		zcbor_state_t *state, const struct pvt_spd *input)
{
	zcbor_print("%s\r\n", __func__);
...
static bool encode_repeated_pvt_hdg(
		zcbor_state_t *state, const struct pvt_hdg *input)
{
	zcbor_print("%s\r\n", __func__);
...
static bool encode_repeated_pvt_alt(
		zcbor_state_t *state, const struct pvt_alt *input)
{
	zcbor_print("%s\r\n", __func__);
...
static bool encode_pvt(
		zcbor_state_t *state, const struct pvt *input)
{
	zcbor_print("%s\r\n", __func__);

	bool tmp_result = (((zcbor_map_start_encode(state, 6) && (((((zcbor_uint32_put(state, (4))))
	...
	&& zcbor_present_encode(&((*input)._pvt_spd_present), (zcbor_encoder_t *)encode_repeated_pvt_spd, state, (&(*input)._pvt_spd))
	&& zcbor_present_encode(&((*input)._pvt_hdg_present), (zcbor_encoder_t *)encode_repeated_pvt_hdg, state, (&(*input)._pvt_hdg))
	&& zcbor_present_encode(&((*input)._pvt_alt_present), (zcbor_encoder_t *)encode_repeated_pvt_alt, state, (&(*input)._pvt_alt))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 6))));
		...
		zcbor_state_t *state, const struct message_out_ts *input)
{
	...
	&& ((((*input)._message_out_ts <= 18446744073709551615ULL)) || (zcbor_error(state, ZCBOR_ERR_WRONG_RANGE), false))
		...
		zcbor_state_t *state, const struct message_out *input)
{
	...
	&& (((*input)._message_out_data_choice == _message_out_data_tstr) ? ((zcbor_tstr_encode(state, (&(*input)._message_out_data_tstr))))
	: (((*input)._message_out_data_choice == _message_out_data_float) ? ((zcbor_float64_encode(state, (&(*input)._message_out_data_float))))
	: (((*input)._message_out_data_choice == _message_out_data_int) ? ((zcbor_int32_encode(state, (&(*input)._message_out_data_int))))
	: (((*input)._message_out_data_choice == _message_out_data__pvt) ? ((encode_pvt(state, (&(*input)._message_out_data__pvt))))
	: false)))))
	&& zcbor_present_encode(&((*input)._message_out_ts_present), (zcbor_encoder_t *)encode_repeated_message_out_ts, state, (&(*input)._message_out_ts))) || (zcbor_list_map_end_force_encode(state), false)) && zcbor_map_end_encode(state, 3))));
