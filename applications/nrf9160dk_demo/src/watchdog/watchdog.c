...
LOG_MODULE_REGISTER(watchdog, CONFIG_NRF9160DK_DEMO_LOG_LEVEL);

#define WDT_TIMEOUT_MS CONFIG_NRF9160DK_DEMO_WATCHDOG_TIMEOUT_MSEC
#define WDT_FEED_WORKER_DELAY_MS ((WDT_TIMEOUT_MS)/2)
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	...
	LOG_INF("Watchdog timeout installed. Timeout: %d",
		CONFIG_NRF9160DK_DEMO_WATCHDOG_TIMEOUT_MSEC);
	return 0;
}
