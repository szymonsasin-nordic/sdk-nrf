...
static void button_send(uint8_t button_num, bool pressed)
{
	static char data[3];
...
static void send_signon_message(void)
{
	msg_send(
	     ...
	     "  Getting started can be found here: https://bit.ly/32tXBnD");
}
...
static void connect_to_cloud(const int32_t connect_delay_s)
{
...
static void cloud_connect_work_fn(struct k_work *work)
{
	...
	if (ret != CLOUD_CONNECT_RES_SUCCESS) {
		...
		LOG_INF("Connection response timeout is set to %d seconds.",
		       CONFIG_CLOUD_WAIT_DURATION / MSEC_PER_SEC);
		k_delayed_work_submit_to_queue(&application_work_q,
					       &cloud_reboot_work,
					       CLOUD_CONNACK_WAIT_DURATION);
	}
...
static void cloud_cmd_handler(struct cloud_command *cmd)
{
	if ((cmd->channel == CLOUD_CHANNEL_LED) &&
		...
		uint8_t led1 = (((uint32_t)cmd->data.sv.value) & 0x00FF) != 0;
		uint8_t led2 = (((uint32_t)cmd->data.sv.value) & 0xFF00) != 0;
			 ...
			 void *user_data)
{
	...
	case CLOUD_EVT_READY:
		...
		/* Set up the sensor. */
		button_sensor_init();
		/* Get ready to report status to shadow on AWS IoT */
		modem_data_init();
...
void main(void)
{
	...
	/* Monitor changes to LTE connection */
